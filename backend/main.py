...
# ----- Planning-Level DSS (Decision Support System) -----
# All outputs are estimates intended to support planning decisions.
# They are NOT substitutes for field-based environmental measurements.
# ==============================

def calc_canopy_coverage(diameter_m):
    """Projected canopy area from crown diameter (circular approximation).
    Formula: A = π × (d/2)²
    Assumption: Crown is approximately circular at maturity.
    Limitation: Actual crown shape varies by species and pruning.
    """
    radius = diameter_m / 2
...
def calc_temperature_reduction_range(canopy_coverage_m2, tree_data=None, site_area=None):
    """Estimate localized microclimate cooling potential (pedestrian-level air temperature).

    Methodology:
      - Computes canopy fraction (canopy area / reference site area).
      - Applies a conservative cooling coefficient range derived from
        urban forestry research in tropical/subtropical climates.
      - Adjusts by an evapotranspiration factor based on tree water use.

    Conservative Parameter Ranges:
      - Low-bound cooling coefficient: 0.8 °C per 100% canopy cover
      - High-bound cooling coefficient: 2.0 °C per 100% canopy cover
      These are intentionally conservative for academic defensibility.

    Output: [min_estimate, max_estimate] in °C

    Scope & Limitations:
      - Applies ONLY to the localized microclimate directly beneath
        and immediately downwind of the canopy, NOT to city-wide temperatures.
      - Assumes daytime conditions with moderate wind (< 3 m/s).
      - Does not account for surrounding built environment thermal mass,
        anthropogenic heat sources, or seasonal humidity variation.
      - Values represent mature-canopy steady-state conditions.
    """
    # Reference site area (nominal 100 m² per tree for realistic proportion)
    ...
    # Evapotranspiration modifier (E-factor)
    # Trees with low water requirements tend to have lower transpiration rates,
    # reducing the latent heat cooling component.
    e_factor = 1.0
    if tree_data:
        ...
        if water_req == "low":
            ...
            e_factor = 1.1  # Higher transpiration = slightly more cooling

    # Conservative cooling coefficient range for tropical urban microclimate
    # (Pedestrian-level ambient air, NOT land surface temperature)
    low_coeff = 0.8   # °C reduction at 100% canopy cover (conservative)
    high_coeff = 2.0   # °C reduction at 100% canopy cover (favorable conditions)

    min_reduction = round(canopy_fraction * low_coeff * e_factor, 2)
    max_reduction = round(canopy_fraction * high_coeff * e_factor, 2)

    # Floor values to avoid reporting implausibly small numbers
...
def calc_co2_range(annual_co2_midpoint):
    """Convert a single CO₂ absorption midpoint into an estimation range.

    Rationale:
      CO₂ sequestration rates in the tree database represent estimated annual
      averages for a MATURE tree under favorable growing conditions. Actual
      rates vary significantly due to:
        - Growth phase (young trees absorb less; peak absorption at mid-life)
        - Urban stress factors (soil compaction, limited root space, pollution)
        - Seasonal variation in photosynthetic activity
        - Pruning, disease, and maintenance practices

    Method:
      - Low estimate: 70% of database midpoint (accounts for urban stress)
      - Mid estimate: database midpoint (favorable mature-tree conditions)
      - High estimate: 115% of database midpoint (optimal conditions)

    Output: {"low": float, "mid": float, "high": float}
    """
    return {
...
def calc_green_potential_score(tree_data):
    """Composite Green Potential Score (0–100) using weighted multi-criteria analysis.

    Scoring Formula:
      GPS = (S_co2 × W1) + (S_canopy × W2) + (S_drought × W3) + (S_lifespan × W4) + (S_growth × W5)

    Component Weights (W) and Justification:
      W1 = 0.30 (CO₂ absorption)  – Primary environmental benefit metric
      W2 = 0.25 (Canopy coverage) – Directly affects microclimate cooling & shade
      W3 = 0.15 (Drought tolerance) – Survivability in tropical dry seasons
      W4 = 0.15 (Lifespan) – Long-term carbon storage & ecosystem value
      W5 = 0.15 (Growth rate) – Time-to-benefit for urban planning

    Normalization:
      Each sub-score is normalized to 0–100 using the maximum value
      observed in the tree database as the reference ceiling.

    Limitation:
      Weights are assigned based on general environmental planning priorities.
      Different use cases (e.g., biodiversity vs. carbon) may warrant
    ...
    # Sub-score normalization (reference ceilings from database)
    co2_score = min(co2 / 40 * 100, 100)         # Ref: ~40 kg/yr max in DB
    canopy_score = min(canopy_d / 15 * 100, 100)  # Ref: ~15m diameter max in DB
    lifespan_score = min(lifespan / 200 * 100, 100)  # Ref: 200 years max in DB

    # Categorical sub-scores
...
def calc_monthly_co2(annual_co2, peak_months):
    """Distribute annual CO₂ absorption across 12 months with seasonal weighting.

    Method:
      Peak-growth months receive ~1.5× the baseline monthly rate.
      Off-peak months receive ~0.6× the baseline monthly rate.
      The total across 12 months equals the annual estimate.

    Assumption: Seasonal variation follows a simplified bimodal pattern
    typical of tropical monsoon climates.
    """
...
def compute_scientific_data(tree_data):
    """Aggregate all estimation model outputs for a single tree species."""
...
async def search_trees(q: str = "", category: str = ""):
    ...
    - q: คำค้นหา (ชื่อไทย/อังกฤษ/วิทยาศาสตร์) — ถ้าว่างจะคืนทั้งหมด
    - category: filter ตามประเภท (economic/edible/conservation) — ถ้าว่างจะค้นทุกประเภท
    """
    results = []
    ...
    for cat in categories:
        for tree_data in TREE_DATABASE.get(cat, []):
            # ถ้าไม่มี query → เอาทั้งหมด, ถ้ามี → partial match
            if q_lower == "" or \
...
"""
    ...
    try:
        ...
        # 7. Environmental Impact Summary (Planning-Level Estimates)
